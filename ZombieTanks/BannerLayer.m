...
+(CCScene *) scene
{
	...
	BannerLayer *layer = [[BannerLayer alloc] init];
...
-(id) init{
    ...
    if( self = [super init] ){
        ...
        // the back button
        CCMenuItemImage *backButton = [CCMenuItemImage itemWithNormalImage:@"back_button2.png" selectedImage:@"back_button2_over.png" disabledImage:@"back_button2.png" target:self selector:@selector(game:)];
        ...
        CCSprite *m_bkgd = [CCSprite spriteWithFile:@"bannerScreen.png"];
        ...
        CCMenuItemImage *decline = [CCMenuItemImage itemWithNormalImage:@"nothanks-button.png" selectedImage:@"nothanks-button-over.png" target:self selector:@selector(game:)];
        decline.tag = 1;
        ...
        CCMenu *declineMenu = [CCMenu menuWithItems:nil];
        [declineMenu addChild:decline];
        declineMenu.position =  ccp((winSize.width/2) + 135 , (winSize.height/2) - 105 );
        //
        [self addChild:declineMenu z:11];
...
// NO THANKS BUTTON = GO TO PLAY THE GAME AND GO TO SURVIVE / INSTRUCTIONS SCREEN
-(void) game:(id) sender{
